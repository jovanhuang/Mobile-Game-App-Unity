...
/// GameManager class handles the flow of the MCQ game
public class GameManager : MonoBehaviour {
    ...
    public void Accept()
    {
        ...
        var type // issue
            = (IsFinished) 
            ? ((isCorrect) ? UIManager.ResolutionScreenType.FinishCorrect : UIManager.ResolutionScreenType.FinishIncorrect)
            : ((isCorrect) ? UIManager.ResolutionScreenType.Correct : UIManager.ResolutionScreenType.Incorrect);

        // if (IsFinished) {
        //     if (isCorrect) {
        //         type = UIManager.ResolutionScreenType.FinishCorrect;
        //     }
        //     else {
        //         type = UIManager.ResolutionScreenType.FinishIncorrect;
        //     }
        // }
        // else {
        //     if (isCorrect) {
        ...
        if (type != UIManager.ResolutionScreenType.FinishCorrect && type != UIManager.ResolutionScreenType.FinishIncorrect)
        {
    ...
    void LoadData()
    {
        ...
        Debug.Log("Scene " + whatSceneWhatQuestion.sceneNumber);

        if (whatSceneWhatQuestion.sceneNumber == 3) {
            var path = Path.Combine(Application.streamingAssetsPath + "/Jovan.xml");
            data = Data.Fetch(path);
        }

        else if (whatSceneWhatQuestion.sceneNumber == 1) {
            var path = Path.Combine(Application.streamingAssetsPath + "/JunLe.xml");
            data = Data.Fetch(path);
        }

        else if (whatSceneWhatQuestion.sceneNumber == 4) {
            var path = Path.Combine(Application.streamingAssetsPath + "/Patria.xml");
            data = Data.Fetch(path);
        }

        else if (whatSceneWhatQuestion.sceneNumber == 2) {
            var path = Path.Combine(Application.streamingAssetsPath + "/Jannat.xml");
            data = Data.Fetch(path);
        }
    }
